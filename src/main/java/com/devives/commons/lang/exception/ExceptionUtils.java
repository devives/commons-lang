...
import com.devives.commons.lang.StringUtils;
...
public class ExceptionUtils {
    ...
    public static String getStackTrace(String message) {
        return getMessageWithStackTrace_(new Exception(StringUtils.getNotNullOrEmpty(message, "Stack trace")));
    }
    ...
    private static String getMessageWithStackTrace_(Throwable throwable) {
        ...
        if (throwable != null) {
            if (StringUtils.notNullOrEmpty(throwable.getMessage())) {
    ...
    /**
     * Метод подавляет указанные типы исключений, остальные пробрасывает.
     *
     * @param func      Анонимный метод.
     * @param exClasses Типы обрабатываемых исключений.
     */
    @SafeVarargs
     ...
     * Если в колекции одно исключение, оно будет выброшено.
     * Если в колекции несколько исключений, будет выброшено исключение, агрегирующее все собранные исключения
    ...
    static public <E extends Throwable> void throwCollected(Collection<E> collection, String message) /*throws E*/ {
        switch (collection.size()) {
            ...
            default:
                AggregateException aggregateException = new AggregateException(StringUtils.getNotNullOrEmpty(message, DEFAULT_AGGREGATE_EXCEPTION_MESSAGE));
    ...
    /**
     * ДЕЛАЕТ ВИД что возвращает исключение обёрнутое либо скастованное к unchecked исключения.
     ...
     * <p>
     * В отличии от throwAsUnchecked позволяет использовать конструкции вида
     ...
     */
    public static <T extends RuntimeException> T asUnchecked(final Throwable throwable) {
     ...
     * </pre>
     */
    static public <R> R passChecked(FailableFunction<R, Exception> func) {
        try {
            return func.apply();
        } catch (Exception e) {
            throw asUnchecked(e);
     ...
     * </pre>
     */
    static public void passChecked(FailableProcedure<Exception> proc) {
        try {
            proc.accept();
        } catch (Exception e) {
            throw asUnchecked(e);
