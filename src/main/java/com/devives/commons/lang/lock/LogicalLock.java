...
public class LogicalLock implements AutoLock {
     ...
     */
    public void ifNotLockedLockAndCall(ExceptionProcedure proc) throws Exception {
        if (notLocked()) {
     ...
     */
    public void lockAndCall(ExceptionProcedure proc) throws Exception {
        try (AutoCloseable ignore = this.lock()) {
            proc.accept();
        }
    }
     ...
     * @throws Exception exception thrown from anonymous method.
     */
    public <R> R lockAndCall(FailableFunction<R, Exception> func) throws Exception {
        try (AutoCloseable ignore = this.lock()) {
            return func.apply();
        }
    }
