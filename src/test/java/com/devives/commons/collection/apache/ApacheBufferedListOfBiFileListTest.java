...
public class ApacheBufferedListOfBiFileListTest<E> extends ApacheAbstractListTest {
    ...
    public List<E> makeObject() {
        return ExceptionUtils.passChecked(() -> {
            CloseableList fileList = SerializedLists.of(new ObjectBinarySerializer(128)).setBiFileStorePath(tempPath).build();
            fileListList_.add(fileList);
            BufferedList<E> list = BufferedLists.of(fileList).setBufferList(new ArrayList<>()).build();
